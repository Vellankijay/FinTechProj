...
export default function CompanyList() {
  ...
  const fetchPortfolio = async () => {
    setLoading(true);
    try {
      const res = await fetch("http://localhost:8000/api/tech/companies");
  ...
  const handleAdd = async () => {
    ...
    try {
      setUpdating(true);
      const res = await fetch("http://localhost:8000/api/tech/add", {
        method: "POST",
  ...
  const removeTicker = async (ticker: string) => {
    try {
      ...
      await fetch(`http://localhost:8000/api/tech/remove/${ticker}`, {
        method: "DELETE",
      });
    } catch (err) {
      console.error(err);
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      setUpdating(true);
      await fetch("http://localhost:8000/api/tech/update-shares", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload),
      });
